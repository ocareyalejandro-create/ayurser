...
## Project Structure
- `app/` — Next.js 14 App Router. `globals.css` (design tokens), `layout.tsx` (next/font), `page.tsx` (the check-in, client component), `checkin.module.css`, `api/check-in/route.ts` (server-side save endpoint), `icon.png` (favicon, reused from the logo → Next emits `<link rel="icon">`).
- `lib/` — `engine.ts` (the heart, pure) + `engine.test.ts`; edges: `db.ts` (server Postgres), `device.ts`, `saveCheckIn.ts`, `name.ts` (+`name.test.ts`) — localStorage name (`ayurser_name`); `cleanName` is pure (trim/collapse/cap 24/capitalize), unit-tested.
- `migrations/0001_check_ins.sql` — the journal's memory (plain Postgres).
- `public/logo.png` — the real tree-of-life meditator logo (922×922 RGBA, TRANSPARENT bg), extracted from the design comp bundle. Rendered via `next/image`: large on the home/settle screen, small/faint (opacity .26) top-right of the result card.
...
- The check-in UI (`app/page.tsx`) — RE-SKINNED to the approved Claude Design comp (harvested from `~/Downloads/Ayurser (2).html`, decoded from its bundled `__bundler/template`). Flow: **naming** (first visit only — "What shall we call you?", underlined serif input, dimmed-until-ready Continue; stores `ayurser_name`) → **home/intro** (logo breathing + BIG serif greeting "Welcome back, {name}." + smaller sage-italic "How do you feel today?" + Begin + bottom-pinned teaser) → 5 questions (breathing progress, "One of five" labels, gentle Back) → result card (read-out sentence, ONE loud anchor in a warm panel, quiet Eat/Ritual/Breath/Move with "More" expander → cited depth, epigraph, separated disclaimer). Below the card: the **Wisdom** opt-in card (dosha explainer — three forces, Prakriti/Vikriti — BEHIND the tap, never up front) + the **practitioner teaser** ("Wisdom / Find a practitioner — Coming soon", honest). State machine: `loading`→`naming|intro`→`question`→`result` (loading resolves the stored name on mount, no flash). All comp placeholder copy REJECTED; substance is our engine + cited content. All styling in `checkin.module.css`.
...
## Current State
- All passing: 34 tests (28 engine + 6 name; `npx vitest run`), `npx tsc --noEmit` clean. Dev server runs on :3000 (coordinator-managed). ⚠️ Do NOT run `next build` while `next dev` is live — it clobbers `.next` and 500s the dev server (happened this session; recovered the main route via a source touch, but the `app/icon.png` metadata route stayed broken on the dev server until a clean restart). Favicon `<link rel="icon">` IS correctly emitted; `/icon.png` will serve once the dev server next does a clean compile/restart.
