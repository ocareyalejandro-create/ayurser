...
## Current State
- All passing: 28 engine tests (`npx vitest run`), `npx tsc --noEmit` clean, dev server on :3000 serves 200. (Did NOT re-run `next build` this session — type+test green.)
- Working tree clean. 4 new focused commits on `feat/next-foundation` (incl. Ritual restore + "More" label). NOT merged, NOT deployed.
